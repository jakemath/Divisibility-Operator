...
bigint::bigint (std::string num)    // String constructor
{
...
{
    if (b.data.back() != 0) // In this program we mark b as 0 by making the most significant
    {                       // digit 0. If it is 0 --> no execution
        ...
        { // carry stores potential overflow at each digit, temp holds result of sum at each digit
            short carry = 0, temp;
            std::list<short>::iterator i = sum.data.begin(), j = b.data.begin();
...
void operator - (bigint& diff, bigint& b)  // Overloaded subtraction. Result stored in diff argument
{
    if (b.data.back() != 0) // No execution if subtracting 0
    {
        ...
        else
        {
            ...
            for (std::list<short>::iterator i = diff.data.begin(), j = b.data.begin(); i != diff.data.end() && j != b.data.end(); ++i, ++j)
            {
                temp = *i - *j;
...
void bigint::operator ++()  // Overloaded pre-increment.
{
    ...
    {
        std::list<short>::iterator i = data.begin();
...
bigint operator * (const bigint& b, int k)  // General overloaded multiplication
{
    ...
    { // Temp holds result of digit multiplication, carry holds potential overflow from temp
        short temp, carry = 0;
        for (std::list<short>::const_iterator i = b.data.begin(); i != b.data.end(); ++i)
        {
...
void bigint::make_rule_multiply (short k)   // Specialized multiplication member function for make_rule.
{                                           // k will always be 3 when this is called. Similar principle
    short temp, carry = 0;
    for (std::list<short>::iterator i = data.begin(); i != data.end(); ++i)
    {
        temp = (*i * k) + carry;
        if (temp > 9)
        {
...
bigint operator * (const bigint& b1, const bigint& b2)    // Bigint multiplication, similar principle as above
{
    ...
    {
        short temp, carry = 0;
        for (std::list<short>::const_iterator i = b1.data.begin(); i != b1.data.end(); ++i)
        {
...
{                                             // Result stored in product and rule remains unchanged. Similar principle as above
    ...
    else
    {
        ...
        for (i = rule.data.begin(), j = product.data.begin(); i != rule.data.end() && j != product.data.end(); ++i, ++j)
        {
            temp = (abs (*i) * k) + carry;
            if (temp > 9)
            {
        ...
        if (i == rule.data.end() && j != product.data.end())
        {
            if (carry != 0)
            {
                *j = carry;
                ...
                --j;
            std::list<short>::iterator k = std::next (product.data.end(), -1);
            for (; j != k;)
            {
...
bool bigint::operator < (const bigint& b) const // Comparison operators
{
    ...
    if (data.back() < b.data.back())
        return true;
    for (std::list<short>::const_reverse_iterator i = ++data.rbegin(), j = ++b.data.rbegin(); i != data.rend() && j != b.data.rend(); ++i, ++j)
    {
        if (*i < *j)
            return true;
        if (*i > *j)
...
bool bigint::operator == (const bigint& b) const
{
    if (data.size() != b.data.size() || data.back() != b.data.back() || data.front() != b.data.front() || negative() != b.negative())
        return false;
    for (std::list<short>::const_iterator i = ++b.data.begin(), j = ++data.begin(); i != b.data.end() && j != data.end(); ++i, ++j)
    {
        if (*i != *j)
            return false;
    }
...
bigint make_rule (bigint b)     // Rule generator. Assumes input ending in a 1, 3, 7, or 9
{
    if (b.data.front() % 2 != 0 && b.data.front() != 5)
    {
        if (b.data.front() == 1)    // If ones = 1 --> truncate ones, negate
        {
            b.data.pop_front();
            b.negate();
        }
        else if (b.data.front() == 9)   // If ones = 9 --> truncate ones, increment
        {
            if (b.data.size() == 1) // Edge case: 9
                return bigint("1");
            else
            {
                b.data.pop_front();
        ...
        else    // If ones != 9 or 1 --> ones = 3 or 7 --> 3rd multiple will end in 1 or 9
        {
            b.make_rule_multiply (3);   // Multiply by 3
            if (b.data.front() == 1)    // If ones = 1 --> truncate ones, negate
            {
                b.data.pop_front();
                b.negate();
            }
            else    // If ones = 9 --> truncate ones, increment
            {
                b.data.pop_front();
                ++b;
            }
    ...
    std::cout << "Error: Only odd divisors allowed." << std::endl;
    exit(1);
}
...
bool div (bigint b1, bigint b2)   // Div operator
{
    if (b2.data.size() == 1 && b2.data.back() != 7 && b2.data.back() != 9)    // Special cases: 2, 3, and 5
    {
        ...
        if (b2.data.front() == 3)   // div by 3
        {
            ...
            for (std::list<short>::iterator i = b1.data.begin(); i != b1.data.end(); ++i)
                total += *i;
            if (total % 3 == 0)
                return true;
            return false;
    ...
    else if (b1.data.size() >= b2.data.size())
    {
        ...
        multiples.insert (bigint("0")); // Insert 0 and b2
        multiples.insert (b2);
        for (int i = 2; i <= 27; ++i)   // Compute and store multiples
        ...
        bigint rule = make_rule (b2), product (rule.data.size() - 1, 0);
